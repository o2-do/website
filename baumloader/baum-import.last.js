...
export const STANDARD = {
  ...
  holz: {
    ...
    stammDicke:   100,        // 40 … 200 %, die Stärke ganz unten am Fuß
    astDicke:     100,        // 40 … 200 %, die Stärke der Hauptäste am Stamm
...
export function normiere(src){
  ...
  return {
    ...
    holz: {
      ...
      // Beide gehen jetzt auch unter 100 %. Eine ältere Datei liegt ohnehin
      // darüber, und wo die Astdicke fehlt, steht sie auf 100 und ändert nichts.
      stammDicke:    cl(num(z.stammDicke, 100), 40, 200),
      astDicke:      cl(num(z.astDicke,   100), 40, 200),
...
export function baueSkelett(cfg, huelle){
  ...
  const reihe = [];
  {
    ...
    // Der rohe Halbmesser wird gemerkt, ehe die Stammdicke ihn aufschlägt.
    // In ihm ist der Querschnitt additiv — r² eines Knotens ist genau die
    // Summe der r² aller Spitzen unter ihm —, und nur darin lässt sich
    // später sauber abziehen, was gestrichen wird.
    for (const k of nd) k.rRoh = k.r;
  }
  // --- Stamm- und Astdicke ---------------------------------------------------
  // Ein Faktor auf das, was das Pipe-Modell gerechnet hat, und er hängt an der
  // Stärke selbst: an den Zweigspitzen bleibt alles, wie es ist, nach innen
  // greift er immer weiter durch. Ein gleichmäßiger Faktor täte es nicht — der
  // machte aus jedem Zweig einen Knüppel und ließe das Verhältnis, in dem der
  // Baum steht, unverändert.
  //
  // Es sind zwei Regler, und sie greifen an zwei Stellen der Kurve an:
  //
  //   Astdicke     dort, wo die Hauptäste stehen — die Stärke, mit der ein Ast
  //                am Stamm ankommt.
  //   Stammdicke   ganz unten am Fuß.
  //
  // Dazwischen und nach außen läuft es linear, mit der Zweigspitze als drittem,
  // festem Punkt. Damit lassen sich beide unabhängig einstellen: ein schlanker
  // Stamm mit kräftigen Hauptästen ist ebenso möglich wie ein mächtiger Fuß,
  // aus dem dünne Äste kommen.
  //
  // Beide gehen unter 100 %: dann wird das Holz dünner als die reine
  // Flächenerhaltung. Sie ist eine Regel für lebende Bäume, keine Vorschrift
  // für gezeichnete — ein Baum darf schlanker stehen, als die Physik verlangt.
  //
  // Maß ist der Halbmesser der Wurzel, also der dickste im Baum. Er wird
  // deshalb vor dem Faktor gemerkt und danach neu genommen.
  //
  // Alles steht als Funktion da und nicht als einmalige Schleife: nach dem
  // Streichen ändern sich die rohen Halbmesser noch einmal, und dann muss neu
  // gerechnet werden — samt seinem Maß, denn auch die Wurzel ist dann dünner.
  const stammF = cl(num(cfg.holz.stammDicke, 100), 40, 200) / 100;
  const astF   = cl(num(cfg.holz.astDicke,   100), 40, 200) / 100;

  // Wo die Hauptäste stehen, als Anteil am Stammhalbmesser. Das ist kein
  // fester Wert: bei zwei Hauptästen trägt jeder die Hälfte der Fläche und
  // steht damit auf 0,71 des Stammes, bei acht auf 0,35. Der Regler wandert
  // deshalb mit und meint immer dieselbe Stelle am Baum.
  const astStelle = () => {
    const R = Math.max(nd[0].rRoh, 1e-6);
    let summe = 0, zahl = 0;
    for (const k of achse)
      for (const c of nd[k].children)
        if (!nd[c].stammPfad && !nd[c].weg){ summe += nd[c].rRoh; zahl++; }
    return zahl ? cl(summe / zahl / R, 0.05, 0.95) : 0.5;
  };
  ...
  const staerken = () => {
    ...
    const tA = astStelle();
    for (const k of nd){
      const t = cl(k.rRoh / R, 0, 1);
      k.r = k.rRoh * (t <= tA ? 1 + (astF - 1) * (t / tA)
                              : astF + (stammF - astF) * ((t - tA) / (1 - tA)));
    }
  };
  staerken();
  let wurzelR = Math.max(nd[0].r, 1e-6);
  ...
  // --- Fortsetzung oder Ansatz -----------------------------------------------
  // Nach dem Ordnen steht fest, welches Kind den Strang weiterführt: das erste.
  // In ihm läuft dasselbe Holz weiter, nur dünner. Jedes weitere Kind ist ein
  // Ansatz — ein eigener Ast, der seitlich herauskommt.
  //
  // Der Unterschied entscheidet, mit welcher Stärke ein Segment anfängt, und
  // das ist mehr als eine Feinheit: ein Ast, der unten am Stamm ansetzt, fing
  // bisher mit Stammstärke an. Gemessen an einem Baum mit 13,5 cm Stammhalb-
  // messer und einem Ast von 7,8 cm liefen die vier Unterteilungspunkte des
  // ersten Segments über 12,4 · 11,2 · 10,1 · 9,0 cm — der Ast stand also fast
  // stammdick am Stamm und verjüngte sich auf einer Segmentlänge auf die
  // Hälfte. Oben, wo die Achse selbst schon dünn ist, fiel es nicht auf; unten,
  // bei tief gezogenem Zentrum, wurde daraus der Klumpen am Ansatz.
  const fortsetzung = new Set();
  for (const k of nd) if (k.children.length) fortsetzung.add(k.children[0]);

  // Der Halbmesser, mit dem ein Ansatz aus seinem Elter tritt. Dieselbe Regel
  // steht in baueHolz() für den Ansatzring: etwas mehr als der Ast selbst,
  // damit er nicht als Rohr in der Rinde steckt, aber nie die Stärke des
  // Elters — sonst wäre es kein Ansatz, sondern eine Fortsetzung.
  const ansatzR = (rElter, rKind) =>
    Math.min(rKind * 1.3, Math.max(rKind, rElter * 0.88));
  ...
  function unterteile(c, anz){
    ...
    const p = nd[c].parent;
    if (p < 0) return;
    const a = nd[p].pos, b = nd[c].pos;
    const rB = nd[c].r, qB = nd[c].rRoh;
    // Eine Fortsetzung erbt die volle Stärke des Elters — dort läuft dasselbe
    // Holz weiter. Ein Ansatz fängt mit seiner eigenen an und wächst normal in
    // den Stamm hinein, statt sich auf dem ersten Segment aus ihm
    // herauszuschälen.
    const weiter = fortsetzung.has(c);
    const rA = weiter ? nd[p].r    : ansatzR(nd[p].r,    rB);
    const qA = weiter ? nd[p].rRoh : ansatzR(nd[p].rRoh, qB);
    let vor = p;
    for (let i = 1; i <= anz; i++){
      const t = i / (anz + 1);
  ...
  let gestrichen = 0;
  {
    ...
    const loese = i => {
      const e = nd[i].parent;
      // Erst die Stärke zurückgeben, solange der Weg zum Boden noch steht.
      //
      // Im Pipe-Modell ist der Querschnitt additiv: r² eines Knotens ist die
      // Summe der r² aller Spitzen unter ihm. Der ganze gestrichene Teilbaum
      // steckt damit als r² seiner Wurzel in jedem Knoten darunter — einmal
      // abziehen genügt, und es ist nicht genähert, sondern genau das, was das
      // Pipe-Modell gerechnet hätte, wenn der Ast nie da gewesen wäre.
      //
      // Ohne das behält der Elter die Stärke für Holz, das nicht mehr da ist.
      // Am Ansatz sitzt dann ein Klumpen: unten der volle Durchmesser, oben
      // der dünne Rest, und dazwischen die Stufe.
      const flaeche = nd[i].rRoh * nd[i].rRoh;
      for (let a = e; a >= 0; a = nd[a].parent)
        nd[a].rRoh = Math.sqrt(Math.max(R_SPITZE * R_SPITZE,
                                        nd[a].rRoh * nd[a].rRoh - flaeche));
      if (e >= 0){
    ...
    // Die Stammdicke noch einmal darüber, mit dem neuen Maß: der Baum trägt
    // weniger und ist deshalb überall dünner, bis hinunter zum Boden.
    //
    // Bewegt hat sich dabei nichts — Schwerkraft und Knorrigkeit sind längst
    // gerechnet und bleiben, wie sie sind. Nur das Holz wird schlanker.
    if (gestrichen){ staerken(); wurzelR = Math.max(nd[0].r, 1e-6); }
  }
