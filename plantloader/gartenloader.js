 ...
 *   Pflanze.buildMaterial(opt)   -> MeshStandardMaterial | [Blatt, Kolben]  (optional, s.u.)
 *
 ...
 *   • Trägt die Pflanze Kolben ($kolben_anteil > 0), stecken sie in DERSELBEN
 *     Geometrie, aber in einer zweiten Zeichengruppe. Ein Mesh genügt also
 *     weiterhin; `buildMaterial` liefert dann ein Array [Blatt, Kolben] statt
 *     eines einzelnen Materials. Wer sein eigenes, einzelnes Material setzt,
 *     bekommt von three.js die Gruppen ignoriert — dann sieht der Kolben aus
 *     wie ein Blatt, aber nichts geht kaputt.
...
(function (root) {
  ...
  function buildPlant(opt) {
    ...
    return {
      ...
      stats: {
        ...
        vertices: s.vertices,
        blades: s.blades,
        kolben: s.kolben                                 // Anzahl der Kolben, 0 = keine
      }
    };
  }
   ...
   *
   * Rückgabe ist ein einzelnes Material — oder, sobald die Pflanze Kolben trägt,
   * das Array [Blatt, Kolben] passend zu den beiden Zeichengruppen der Geometrie.
   * Die Kolbenfarbe steckt in den Vertexfarben; das zweite Material unterscheidet
   * sich vom ersten nur dadurch, dass es die Halm-Textur NICHT bekommt.
   */
  function buildMaterial(opt) {
    ...
    if (!cfg || !(cfg.kolben_anteil > 0)) return mat;

    // Der Kolben ist ein geschlossener Sechskant — Vorderseite genügt ihm.
    var kolben = new root.THREE.MeshStandardMaterial({
      color: 0xffffff,
      vertexColors: true,
      side: root.THREE.FrontSide,
      roughness: mat.roughness,
      metalness: 0.0,
      envMapIntensity: mat.envMapIntensity
    });

    // Eigene Textur, um den Kolben gewickelt. Kein alphaTest: ein Loch in einem
    // geschlossenen Körper gäbe den Blick auf seine Innenseite frei.
    if (cfg.kolben_textur) {
      var ktex = new root.THREE.TextureLoader().load(cfg.kolben_textur);
      if ('colorSpace' in ktex) ktex.colorSpace = root.THREE.SRGBColorSpace;
      else ktex.encoding = root.THREE.sRGBEncoding;
      kolben.map = ktex;
    }
    return [mat, kolben];
  }
