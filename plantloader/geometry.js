...
(function (root, factory) {
  ...
  /**
   * Halmnummer -> Pseudozufallszahl in [0,1).
   *
   * Bewusst kein Math.random(): der Generator sagt zu, dass dieselbe
   * Konfiguration immer dieselben Vertexdaten liefert. Ein echter Zufall würde
   * bei jedem "Generieren" ein anderes Büschel ergeben und im Garten bei jedem
   * Laden ein anderes. Der Hash streut trotzdem gleichmäßig.
   */
  function hash01(n) {
    n = (n ^ 0x9e3779b9) >>> 0;
  ...
  function buildTuft(cfg, opts) {
    ...
    var kuerzung = Math.max(0, cfg.zufall_verkuerzung || 0);
    ...
    for (var b = 0; b < nBlades; b++) {
      ...
      if (hasLeaf) {
        // Ziellänge dieses Halms — die zufällige Kürzung greift HIER, ganz am
        // Anfang. Alles Weitere hängt daran: Segmentlängen, die Breitenkurve
        // (sie rechnet in t = 0…1 und skaliert dadurch mit), der Laufweg für
        // Farbverlauf und Textur und damit auch der Sitz des Kolbens.
        var L = (kuerzung > 0 ? Math.max(0, H - hash01(b) * kuerzung) : H) / nSeg;
    ...
    /**
     * Ein Dreieck aus drei Punkten [x,y,z] mit eigener UV je Ecke. Weil nichts
     * geteilt wird, mittelt computeNormals nichts weg — der Sechskant behält
     * seine harten Kanten.
     *
     * Die UV umläuft den Kolben: u von 0 bis 1 einmal um die sechs Flächen
     * herum, v von 0 an der unteren bis 1 an der oberen Spitze. Damit lässt
     * sich $kolben_textur wie ein Etikett aufziehen.
     */
    function putTri(p1, u1, v1, p2, u2, v2, p3, u3, v3, col) {
      putVertex(vi, p1[0], p1[1], p1[2], u1, v1, col[0], col[1], col[2]);
      putVertex(vi + 1, p2[0], p2[1], p2[2], u2, v2, col[0], col[1], col[2]);
      putVertex(vi + 2, p3[0], p3[1], p3[2], u3, v3, col[0], col[1], col[2]);
    ...
    function bounds(p) {
      ...
      if (p[1] < minHeight) minHeight = p[1];
    ...
    function buildKolben() {
      ...
      for (var t = 0; t < kolbenTips.length; t += 8) {
        ...
        // Textur-V an den vier Höhenstufen
        var v0 = 0, v1 = tipH / kolbenL, v2 = (tipH + shaft) / kolbenL, v3 = 1;
