...
(function (root, factory) {
  ...
  var PARAMS = [
    ...
    { key: 'zufall_verkuerzung', type: 'range', min: 0, max: 1000, step: 5, def: 0,
      group: 'Höhen', unit: 'mm', label: 'Zufällig verkürzen',
      hint: 'Jeder Halm wird um 0 … diesen Wert gekürzt. Die Kürzung greift ganz am ' +
            'Anfang an der Ziellänge, der Halm wird dadurch als Ganzes kleiner — ' +
            'Breitenkurve, Farbverlauf und der Sitz des Kolbens gehen mit. ' +
            'Ergibt eine natürlichere Wuchsform, vor allem bei aufrechten Halmen mit Kolben. ' +
            'Der "Zufall" ist fest an die Halmnummer gekoppelt: dieselbe Konfiguration ' +
            'ergibt immer dasselbe Büschel.' },
    ...
    // --- Kolben ---
    // Voreinstellung 0 %: eine Datei ohne diese Felder ergibt damit dieselbe
    // Pflanze wie vor der Erweiterung. Mehr Abwärtskompatibilität braucht es
    // nicht — normalize() füllt fehlende Felder ohnehin mit dem Standardwert.
    { key: 'kolben_anteil', type: 'range', min: 0, max: 100, step: 1, def: 0,
      group: 'Kolben', unit: '%', label: 'Kolben',
      hint: 'Anteil der Halme, die oben einen Kolben tragen. 0 % = keine Kolben, ' +
            '100 % = jeder Halm. Die Auswahl ist gleichmäßig verteilt und ohne Zufall — ' +
            'dieselbe Konfiguration ergibt immer dieselben Halme.' },
    { key: 'kolben_durchmesser', type: 'range', min: 1, max: 200, step: 0.5, def: 12,
      group: 'Kolben', unit: 'mm', label: 'Kolbendurchmesser',
      hint: 'Durchmesser des Sechskants über die Ecken (Umkreis). Über die Flächen ' +
            'gemessen ist er um den Faktor 0,866 kleiner.' },
    { key: 'kolben_laenge', type: 'range', min: 1, max: 1000, step: 5, def: 120,
      group: 'Kolben', unit: 'mm', label: 'Kolbenlänge',
      hint: 'Gesamtlänge einschließlich beider Spitzen. Eine Spitze ist bei 60° ' +
            'Öffnungswinkel gut 0,87 × Durchmesser hoch; bleibt dafür kein Platz, ' +
            'werden die Spitzen spitzer und die Länge stimmt trotzdem.' },
    { key: 'kolben_farbe', type: 'color', def: '8a5a2b',
      group: 'Kolben', label: 'Kolbenfarbe', hint: '6 Zeichen Hex, ohne Raute.' },
    { key: 'kolben_textur', type: 'image', def: null,
      group: 'Kolben', label: 'Kolben-Textur (optional)',
      hint: 'Wird wie ein Etikett um den Kolben gezogen: waagerecht einmal um die ' +
            'sechs Flächen herum, senkrecht von der unteren bis zur oberen Spitze. ' +
            'Sie wird mit der Kolbenfarbe multipliziert — für das reine Bild die ' +
            'Farbe auf ffffff stellen.' },
